...
class TCGA_Data():
    def __init__(self, args):
        ...
        # self.patients_temp = self.tcga_data['rnaseq']
    ...
    def normalize_data(self, patient_features):
        # a = np.max(patient_features, axis=0)
        # b = np.min(patient_features, axis=0)
    ...
    def generate_dataset(self, patient_features, num_treatments):
        ...
        #     t, dosage, y = generate_patient(x=patient_features[i], v=self.v, num_treatments=num_treatments,
        #                                     treatment_selection_bias=self.treatment_selection_bias,
        #                                     dosage_selection_bias=self.dosage_selection_bias,
        #                                     scaling_parameter=self.scaling_parameteter,
        #                                     noise_std=self.noise_std)
        #
        #     l = []
        #     l.append(patient_temp[i])
        #     temp = l[0].tolist()
        #     temp2 = np.linspace(1, 10, 2000).tolist()
        #     temp.extend(temp2)  #添加其他一些无关的特征
        #     temp = [np.array(temp)]
        #     tcga_dataset['x'].append(temp)
        #
        #
        #
        #     tcga_dataset['t'].append(t)
        ...
        #     tcga_dataset['y'].append(y)
        #
        # tcga_dataset['x'] = self.normalize_data(temp) #等会儿注释掉
