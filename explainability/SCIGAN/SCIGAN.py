...
class SCIGAN_Model:
    def __init__(self, params):
        ...
        self.SCIGAN_iteration = params['SCIGAN_iteration']
        self.inference_iteration = params['inference_iteration']

        # tf.reset_default_graph() #AttributeError: module 'tensorflow' has no attribute 'reset_default_graph'
        tf.compat.v1.reset_default_graph()
        tf.random.set_random_seed(10) #源代码为tensorflow-gpu==1.15.0版本，但tf2.0+版本中去除了1.0中很多函数
    ...
    def generator(self, x, y, t, d, z, treatment_dosage_samples):
        with tf.variable_scope('generator', reuse=tf.AUTO_REUSE):
            ...
            for treatment in range(self.num_treatments):
                ...
                treatment_layer_1 = tf.layers.dense(input_counterfactual_dosage, self.h_dim, activation=tf.nn.tanh,
                                                    name='treatment_layer_1_%s' % str(treatment), reuse=tf.AUTO_REUSE)
                ...
                treatment_layer_2 = tf.layers.dense(treatment_layer_1, self.h_dim, activation=tf.nn.tanh,
                                                    name='treatment_layer_2_%s' % str(treatment), reuse=tf.AUTO_REUSE)

                # treatment_dosage_output = tf.layers.dense(treatment_layer_2, 1, activation=None,
                #                                           name='treatment_output_%s' % str(treatment),
                #                                           reuse=tf.AUTO_REUSE)
                treatment_dosage_output = tf.layers.dense(treatment_layer_2, 1, activation=tf.nn.tanh,
                                                          name='treatment_output_%s' % str(treatment),
                                                          reuse=tf.AUTO_REUSE)
    ...
    def train(self, Train_X, Train_T, Train_D, Train_Y, verbose=False):
        ...
        for it in tqdm(range(self.SCIGAN_iteration)):
            for kk in range(2):
            ...
            # if it % 1000 == 0:
                ...
                # print('G_dosage_outcome:{}'.format(G_treatment_dosage_outcomes_curr))
                print()

        # Train Inference Network
        print("Training inference network.")
        for it in tqdm(range(self.inference_iteration)):
            idx_mb = sample_X(Train_X, self.batch_size)
            X_mb = Train_X[idx_mb, :]
            T_mb = np.reshape(Train_T[idx_mb], [self.batch_size, ])
            D_mb = np.reshape(Train_D[idx_mb], [self.batch_size, ])
