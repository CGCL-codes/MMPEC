...
# from news_data_simulation import get_dataset_splits, TCGA_Data
from covid_data_simulation import get_dataset_splits, TCGA_Data
from covid_inference import inference

from SCIGAN import SCIGAN_Model
from utils.evaluation_utils import compute_eval_metrics

os.environ["CUDA_VISIBLE_DEVICES"] = "-1"  #使用CPU运行代码
os.environ['TF_CPP_MIN_LOG_LEVEL'] = '2' #设置log输出信息
tf.compat.v1.logging.set_verbosity(tf.compat.v1.logging.ERROR)


def init_arg():
    parser = argparse.ArgumentParser()
    parser.add_argument("--num_treatments", default=3, type=int)  #频繁需要修改的地方
...
if __name__ == "__main__":
    ...
    params = {'num_features': dataset_train['x'].shape[1], 'num_treatments': args.num_treatments,
              ...
              'h_inv_eqv_dim': args.h_inv_eqv_dim, 'SCIGAN_iteration':2000, 'inference_iteration':4000}
    ...
    # model_baseline.train(Train_X=dataset_train['x'], Train_T=dataset_train['t'], Train_D=dataset_train['d'],
    #                      Train_Y=dataset_train['y'], verbose=args.verbose)
    ...
    output_data = inference(dataset, dataset_test['x'], num_treatments=params['num_treatments'],
                                         num_dosage_samples=params['num_dosage_samples'], model_folder=export_dir)
