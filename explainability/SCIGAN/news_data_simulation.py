...
class TCGA_Data():
    def __init__(self, args):
        ...
        # con = sqlite3.connect("datasets/news.db")  # 打开数据库
        # con.row_factory = dict_factory
        # c = con.cursor()
        # c.execute("select x from news")
        # # output = c.fetchmany(1000)
        # output = c.fetchmany(5)
        # print(output)
        # # output_data = pickle.loads(output)
        # while output:
        #     data_temp = c.fetchmany(1000)
        #     output.append(data_temp)
        # c.close()
        # #
        #
        #
        # f = open('datasets/tcga.db', 'rb')
