...
def inference(dataset, test_patients, num_treatments, num_dosage_samples, model_folder):
    mises = []
    ...
    # num_integration_samples = 2 ** samples_power_of_two + 1  #生成的dosage个数
    num_integration_samples = dataset['metadata']['d_max']
    # step_size = 1. / num_integration_samples
    treatment_strengths = np.arange(0, num_integration_samples)  #随机生成dose

    with tf.Session(graph=tf.Graph()) as sess:
        tf.saved_model.loader.load(sess, ["serve"], model_folder) #加载已经训练好的模型
        ...
        for patient in test_patients:
            for treatment_idx in range(num_treatments):
                ...
                #使用训练好的模型对test_Data进行推理，得到outcome
                ...
                # pred_dose_response = pred_dose_response * (dataset['metadata']['y_max'] - dataset['metadata']['y_min']) + dataset['metadata']['y_mean']


                test_data['y'] = pred_dose_response * 6
                data.append(test_data['y'])

        output = pd.DataFrame(data)
        output.to_excel("output_test.xlsx",index=False)
